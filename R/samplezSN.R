samplezSN = function(y, N, particles, priorList){
...
#
 ...
 zPars = .C('rzSN', as.double(absz), as.double(M), as.double(RV), as.integer(n), as.integer(p), as.integer(N), as.double(yVec), as.double(xiVec), as.double(psiVec), as.double(GVec), NAOK=TRUE)
 absz = zPars[[1]]
 M = zPars[[2]]
 RV = zPars[[3]]
