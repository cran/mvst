sampleBSN = function(y, X, N, particles, priorList){
# Given the arguments, this function returns a population of MC draws for the values of the variable B, in the p-variate skew-Normal regression model.
 n = nrow(y)
 p = ncol(y)
 k = ncol(X)
 B = array(0, c(k, p, N))
 ...
 for(iN in 1:N){
  ...
  HiN = y - matrix(rep(psi[iN,,drop=F],n),ncol=p,byrow=T) * matrix(rep(abs(ziN),p),ncol=p)
  S.iN = t(X) %*% X
  invS.iN = solve(S.iN)
  Cpsi.iN = t(X) %*% HiN
