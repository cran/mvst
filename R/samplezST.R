samplezST = function(y, X, N, particles, priorList){
# Given the arguments, this function returns a population of MC draws for the values of the latent variable Z, in the p-variate skew-t model.
 ...
 if(XFlag){
  ...
  zPars = .C(C_rzSTX, as.double(absz), as.double(M), as.double(RV), as.integer(n), as.integer(p), as.integer(k), as.integer(N), as.double(yVec), as.double(XVec), as.double(BVec), as.double(psiVec), as.double(GVec), as.double(vVec), NAOK=TRUE)
 } else {
  xiVec = as.numeric(t(particles$xi))
  zPars = .C(C_rzST, as.double(absz), as.double(M), as.double(RV), as.integer(n), as.integer(p), as.integer(N), as.double(yVec), as.double(xiVec), as.double(psiVec), as.double(GVec), as.double(vVec), NAOK=TRUE)
 }
 absz = zPars[[1]]
 M = zPars[[2]]
 RV = zPars[[3]]
 ...
 log.dz = norm.dens - rep(n*log(2), N) - truncs
 Rz = zSigns * absz
